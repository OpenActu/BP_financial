...
⚠️ **Et $\bar\rho$ n'est pas une constante.** En crise, les corrélations convergent vers 1 ([§ 14 du cours de statistique](../statistique/mathematique/14-dependance-et-echec-du-tcl.md)) : le plancher $\sigma\sqrt{\bar\rho}$ remonte vers $\sigma$ au moment précis où l'on comptait dessus. La diversification est une protection **qui s'évapore dans le sinistre** — contrairement à une couverture par future, qui, elle, se renforce.
...
> - $h^\star=\operatorname{Cov}/\operatorname{Var}$ et il subsiste $1-\rho^2$ de variance ($\Rightarrow$ [`modele.md`](../modele/modele.md)) ;
