...
Dans le vocabulaire du [module 3](03-cauchy-schwarz-et-angle.md), c'est le cas $\theta=90°$. Le vecteur nul est orthogonal à tout le monde ; c'est le seul vecteur orthogonal à lui-même (par la propriété « définie positive » du § 2.1).

**Orthogonalité à un sous-espace.** $u\perp F$ signifie $u\perp f$ pour **tout** $f\in F$. Par bilinéarité, il suffit de le vérifier sur une **famille génératrice** de $F$ ([§ 4.3](04-sous-espaces-et-familles-generatrices.md)) : c'est ce qui rend la vérification praticable.

> 📐 **Rappel — famille génératrice.** $g_1,\dots,g_m$ est génératrice de $F$ lorsque $F=\text{Vect}(g_1,\dots,g_m)$, c'est-à-dire lorsque tout $f\in F$ s'écrit $f=\sum_i\lambda_ig_i$. Le [module 4](04-sous-espaces-et-familles-generatrices.md) insiste sur un point dont la démonstration ci-dessous va tirer parti : une telle famille garantit l'**existence** des $\lambda_i$, jamais leur unicité.

Les sous-espaces en jeu sont ceux du [§ 4.4](04-sous-espaces-et-familles-generatrices.md), et le plus important est $\text{Vect}(\mathbf 1,t)$, engendré par $\mathbf 1=(1,\dots,1)$ et $t=(1,2,\dots,n)$ : le plan des séries **affines**, celui où vit la droite ajustée d'une série de clôtures ([module 6](06-projection-orthogonale.md)). La proposition ci-dessous y prend un sens très concret : $u\perp\text{Vect}(\mathbf 1,t)$ ne demande que **deux** vérifications, $\sum_iu_i=0$ et $\sum_i i\,u_i=0$, au lieu d'une infinité.
...
> 🔑 **Une infinité de vérifications se ramène à $m$ produits scalaires.** $F$ est infini dès qu'il n'est pas $\{0\}$ ; la proposition remplace « pour tout $f\in F$ » par $m$ égalités numériques. C'est exactement le mécanisme réutilisé au § 5.2 pour l'hérédité — où $S$ est engendré par $w_1,\dots,w_{k-1}$ — et au [module 6](06-projection-orthogonale.md) pour caractériser la projection.
...
Ce $u$ n'est pas tombé du ciel : c'est le vecteur **centré** de la série $x=(10,\,12,\,11,\,15)$, de moyenne $\bar x=12$, soit $u=x-\bar x\mathbf 1$. Ce n'est pas une coïncidence — *tout* vecteur centré est orthogonal à $\mathbf 1$, puisque $\sum_i(x_i-\bar x)=\sum_ix_i-n\bar x=0$ par définition de la moyenne. C'est l'objet de l'exercice E5.2, la décomposition $x=\bar x\mathbf 1+\tilde x$ du § 5.4, et tout le [module 8](08-degres-de-liberte-et-centrage.md).
...
**Démonstration.** Immédiate par développement (§ 2.2) : $\|u+v\|^2=\|u\|^2+2\langle u,v\rangle+\|v\|^2$. Le terme croisé disparaît si et seulement si $\langle u,v\rangle=0$. $\blacksquare$
...
nulle part ailleurs, qu'elle sert. C'est aussi le mécanisme annoncé au § 5.1 — être orthogonal à
...
Le théorème du § 5.2 s'applique alors au **couple** $(S,w_k)$ :
...
> L'égalité de Pythagore équivaut donc à l'annulation de la **somme** des termes croisés, pas de  chacun d'eux. Contre-exemple dans $\mathbb R^2$ : $w_1=(1,0)$, $w_2=(0,1)$, $w_3=(1,-1)$ ont  pour termes croisés $0$, $1$ et $-1$, de somme nulle ; l'identité tient bien — $\|(2,0)\|^2=4$ et $1+1+2=4$ — alors que $w_1$ et $w_3$ ne sont **pas** orthogonaux. La réciproque gratuite du § 5.2 est un privilège du couple.
...
- **écriture unique** — tout vecteur de $\text{Vect}(w_1,\dots,w_k)$ s'y décompose d'une seule façon. C'est précisément ce qui manquait à la famille génératrice quelconque du § 5.1, où seule l'*existence* des coefficients était acquise ;
...
## 5.4 Ce que le théorème devient sur des données
...
| ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| König–Huygens $\sum_i(x_i-\bar x)^2=\sum_i x_i^2-n\bar x^2$       | Pythagore sur $x=\bar x\mathbf 1+\tilde x$ — [module 7](07-supplementaire-orthogonal-et-dimension.md) |
| Table d'ANOVA : $SC_{\text{tot}}=SC_{\text{exp}}+SC_{\text{rés}}$ | Pythagore sur $y=P_F(y)+(y-P_F(y))$ — [module 6](06-projection-orthogonale.md)                        |

---

## 5.5 Simulation

### S5.1 — Pythagore, dans les deux sens
