"""Moteur de l'experience 5 : l'experience 4, remesuree par les cinq pistes de sa revue.

Rejoue la regle de l'experience 4 et verifie que le portefeuille est identique,
puis applique les cinq pistes retenues : intervalles par grappes de dates, vingt
phases et correction de Holm (T1, avec les apports de C2) ; portefeuille fictif
MENSUEL separant la regle de sa cadence (T3) ; references simulees de la bande
(C1) ; dimensionnement de la regle sur 2021 (T4) ; reference a exposition
appariee, betas semestriels et temoin aleatoire (T2).
...
DOTATION = 10000.0
LIGNES = 5
TE_DECLAREE = 11.08              # T4 : projetee sur 2021 par la regle elle-meme
...
# T4 et T2 : les grandeurs de 2021 publiees au README.
PROJECTIONS_PUBLIEES = {
    "quotidien": {"te": 11.08, "beta": 0.832, "part": 75.5, "vides": 10, "ordres": 34,
                  "frais": 203.65, "duree": 55.5, "ouvertes": 2, "perdus": 75},
    "mensuel": {"te": 10.40, "beta": 0.515, "part": 47.1, "vides": 40, "ordres": 14,
                "frais": 80.18, "duree": 66.5, "ouvertes": 2, "perdus": 0},
    "sans_p20": {"te": 12.52, "beta": 1.040, "part": 87.8, "vides": 10, "ordres": 36,
                 "frais": 230.64, "duree": 61.5, "ouvertes": 4, "perdus": 431},
    "paires": {"fantome": 10.00, "mensuel": 9.76, "mensuel_neutralise": 8.68,
               "beta_mensuel": 0.316, "appariee": 9.08},
    "temoin": {"UNIVERS": 9.81, "TOP10": 8.11},
    "betas_semestres": {"premier": 0.766, "second": 0.883},
}
...
def simuler(ctx, decisions, seances, avec_p20=True):
...
def svg_portefeuille(chemin, dates, courbes, executions, debut):
    ...
    out = [
        ...
        (f'<text x="{marge_g}" y="24" font-size="15" font-weight="600" fill="#1a1a1a">'
         f'Experience 5 &#8212; portefeuille contre {REFERENCE}, base 100 au {debut}</text>'),
    ...
    for rang, mois in enumerate(liste):
        ...
        svg_portefeuille(rep / GRAPHIQUES / f"portefeuille-{mois}.svg", jours, [
            ...
            ("fantôme SANS-P20", [100 * b["valeurs_f"][j][2] / b["dotation"] for j in jours],
             "#8e6bb8", False),
            ("MENSUEL", [100 * b["valeurs_m"][j][2] / b["dotation"] for j in jours],
             "#c0782c", False),
        ...
        bloc = [
            ...
            (f"> Journal de l'[expérience 5](../README.md) · exécutions du {premiere} "
            ...
            (f"| Portefeuille fictif `MENSUEL`, même base "
             f"| {fr(100 * b['valeurs_m'][fin_mois][2] / b['dotation'])} |"),
...
def ecarts_publies(b):
    ...
    for nom, cle_b in (("quotidien", "p_q"), ("mensuel", "p_m"), ("sans_p20", "p_f")):
        for cle, publie in PROJECTIONS_PUBLIEES[nom].items():
            calcule = b[cle_b]["n_ordres"] if cle == "ordres" else b[cle_b][cle]
            confronter(f"projection {nom}", cle, publie, calcule)
    paires = b["paires_p"]
    for cle, calcule in (("fantome", paires["fantome"]["te"]),
                         ("mensuel", paires["mensuel"]["te"]),
                         ("mensuel_neutralise", paires["mensuel"]["te_residu"]),
                         ("beta_mensuel", paires["mensuel"]["beta"]),
                         ("appariee", paires["appariee"]["te"])):
        confronter("projection paires", cle, PROJECTIONS_PUBLIEES["paires"][cle], calcule)
...
def ecarts_etalonnage(b):
    return [(cle, p, v) for groupe, cle, p, v in ecarts_publies(b) if groupe == "etalonnage"]


def survie(verdict_4, verdict_5):
    if verdict_4 == "exclut zéro":
        return "survit" if verdict_5 == "sépare" else "**tombe**"
    if verdict_4 == "contient zéro":
        return "**apparaît**" if verdict_5 == "sépare" else "confirmé"
    return "—"
...
def section_compte(b):
    ...
    out = [
        "## 1. Le compte", "", "| | |", "|---|---|",
        (f"| **Contrôle de reproduction** | {nombre} ordres, {fr(valeur)} {EURO} au {b['fin']} : "
         "**identiques à l'expérience 4**, ordre par ordre et séance par séance |"),
        ...
        "", "## 2. Mois par mois", "",
        (f"| Mois | Valeur | Base 100 | Fantôme | `MENSUEL` | {REFERENCE} | Alpha du mois "
         "| Alpha cumulé | Ordres |"),
        "|---|---|---|---|---|---|---|---|---|",
    ]
    ...
    for mois in sorted(fins):
        ...
        cumul = 100 * total / b["dotation"] - b["ref100"][fin]
        ...
        out.append(
            ...
            f"| {fr(100 * b['valeurs_f'][fin][2] / b['dotation'])} "
            f"| {fr(100 * b['valeurs_m'][fin][2] / b['dotation'])} | {fr(b['ref100'][fin])} "
            f"| {signe(alpha_mois)} pt | {signe(cumul)} pt | {combien or '—'} |")
    return out
...
def section_positions(b):
    ...
    out = [
        ...
        (f"> La contribution est **nette des frais payés** : une position encore ouverte n'a "
         ...
         f"d'exécution**, à l'ouverture, à l'écart des **clôtures** de {REFERENCE}, qui n'a pas "
         "de cours d'ouverture : c'est une convention, déclarée. Le repli maximal se mesure du "
         "prix d'achat au plus bas atteint pendant la détention."), "",
    ...
    out += [
        ...
        (f"| Signaux perdus faute de créneau | {len(faute)}, sur {b['jours_pleins']} "
         "séances |"),
        "",
        ("> **C2, fusionnée dans T1.** Le premier franchissement du bord n'a pas de valeur "
         "d'information ; seule la persistance en a "
         "([module 4, § 4.2](../../../raw/concept/semestre3/canal/04-sorties-de-canal.md"
         "#42--le-bon-critère--la-persistance)). Voici depuis combien de séances chaque valeur "
         "achetée était sous son bord bas, et candidate, le jour de la décision."), "",
        "| Valeur | Décision | Séances sous le bord bas | Séances candidate |",
        "|---|---|---|---|",
    ]
    for o, sous, candidat in b["durees"]:
        out.append(f"| `{o['TICKER']}` {SOCIETES[o['TICKER']]} | {o['DATE_DECISION']} "
                   f"| {sous} | {candidat} |")
    premieres = sum(1 for _o, sous, _c in b["durees"] if sous == 1)
    out += ["", (f"**{premieres} achats sur {len(b['durees'])}** sont décidés à la première "
                 "séance sous le bord bas.")]
    return out
...
def section_univers(ctx, b):
    ...
    out = ["", "## 5. L'univers et la recevabilité des données", "",
           ("> L'univers est celui de l'expérience 4, repris à l'identique dans "
...
def section_etalonnage(b):
    ...
    lignes = [
        ...
        ("Écart-type d'`EXCES_20`, sous-échantillon (pt)", "sd_echantillon", None, 2),
    ]
    out = ["", "## 6. Les taux d'étalonnage de l'expérience 4, recalculés", "",
           "| Mesure | Publié | Moteur, 2021 | Concorde | 2022 |", "|---|---|---|---|---|"]
    for libelle, cle, base, decimales in lignes:
        valeur = publie[cle]
        texte = fr(valeur, 2) if isinstance(valeur, float) else str(valeur)
        out.append(f"| {libelle} | {texte} | {cellule(e, cle, base, decimales)} "
                   f"| {'✗' if cle in ecarts else '✓'} | {cellule(n, cle, base, decimales)} |")
    out += ["", (f"**{len(publie) - len(ecarts)} nombres publiés sur {len(publie)}** sont "
                 "retrouvés à l'identique ; le tableau en reprend sept, tous sont "
                 "confrontés.")]
    return out
...
def section_issues(b):
    ...
    out = [
        "", "## 7. Les issues remesurées — T1", "",
        ("> Mêmes issues, mêmes lignes que l'expérience 4. Ce qui change est leur incertitude : "
         "erreur type **par grappes de dates**, **vingt phases**, **correction de Holm**, et le "
         "verdict déclaré au [protocole](README.md#le-verdict-et-sa-règle) : une comparaison "
         f"sépare son issue si Holm la rejette **et** si au moins {SEUIL_PHASES} phases sur "
         f"{PHASES} excluent zéro avec le même signe."), "",
        "### À la phase 0, celle de l'expérience 4", "",
        ("| Élément | Effectifs | Différence | IC95 Welch | IC95 par grappes | G | p "
         "| Holm | Phases excluant zéro | Verdict de l'expérience 4 | **Verdict** | Survie |"),
        "|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    ...
    out += ["", "### Sur les vingt phases", "",
            ("| Élément | Différence minimale | Différence maximale | Phases mesurables "
             "| Phases excluant zéro |"),
            "|---|---|---|---|---|"]
    for cle, libelle, *_r in COMPARAISONS:
        mesurables = sum(1 for c in ph[cle]["phases"] if c["ic"] is not None)
        out.append(f"| {libelle} | {signe(ph[cle]['min'])} pt | {signe(ph[cle]['max'])} pt "
                   f"| {mesurables} / {PHASES} | {ph[cle]['n_exclut']} / {PHASES} |")
    out += ["", "### Les effets minimaux détectables, projetés contre réalisés", "",
            ("| Élément | EMD projeté par l'expérience 4 | EMD projeté par grappes "
             "| IC95 réalisé par grappes | IC95 par grappes, toutes séances d'audit |"),
            "|---|---|---|---|---|"]
    for cle, libelle, *_r in COMPARAISONS:
        out.append(f"| {libelle} | ± {fr(PROJECTIONS[cle][1], 1)} pt "
                   f"| ± {fr(EMD_GRAPPES[cle], 1)} pt | ± {fr(ph[cle]['phases'][0]['ic'])} pt "
                   f"| ± {fr(b['quotidien'][cle]['ic'])} pt |")
    tombes = [libelle for cle, libelle, *_r in COMPARAISONS
              if survie(verdicts_4[cle], verdicts[cle]) == "**tombe**"]
    separent = [libelle for cle, libelle, *_r in COMPARAISONS if verdicts[cle] == "sépare"]
    nombre, places, places_ech, episodes_ech = b["episodes_top"]
    cand, cand_places, _pe, _ee = b["episodes_candidats"]
    non_tranchees = ph["VENTE"]["phases"][0]["non_tranchees"]
    out += [
        "",
        (f"{non_tranchees} évaluations du sous-échantillon ont un horizon qui dépasse le "
         f"{FIN_SERIE} : elles sont non tranchées et hors des moyennes."),
        "",
        (f"**Effectifs réels.** Les {cand_places} évaluations candidates de l'audit forment "
         f"**{cand} épisodes** ; les {places} places du TOP 10 forment {nombre} séjours, et les "
         f"{places_ech} places du sous-échantillon en touchent **{episodes_ech}**."),
        "",
        ("**Ce qui sépare son issue, sous la convention déclarée : "
         + (", ".join(separent) if separent else "aucun élément de la règle") + ".**"
         + (" Le verdict de l'expérience 4 qui **tombe** : " + ", ".join(tombes) + "."
            if tombes else " Aucun verdict de l'expérience 4 ne tombe.")),
    ]
    return out
...
def section_bande(b):
    ...
    ecarts = [e for e in ecarts_publies(b) if e[0].startswith("reference")]

    def ic(p):
        return f" ± {fr(p['ic'], 1)}" if p["ic"] is not None else ""

    out = [
        "", "## 8. La bande contre ses deux références — C1", "",
        ("> Le README de l'expérience 4 jugeait `BANDE` contre 68,3 % et expliquait l'écart par "
         "le levier, qui joue en sens inverse. Les deux références sont simulées avec "
         "l'estimateur du protocole, et ont été publiées avant la première séance."), "",
        "| | Sous le bord bas | Dans la bande | Au-dessus du bord haut | `BANDE` |",
        "|---|---|---|---|---|",
    ]
    for nom, cle in (("Bruit i.i.d.", "iid"), ("Marche aléatoire", "marche")):
        r = ref[cle]
        out.append(f"| {nom}, {TIRAGES_REFERENCE} tirages | {fr(r['sous'], 1)} % "
                   f"| {fr(r['dans'], 1)} % | {fr(r['dessus'], 1)} % | {fr(r['bande'], 1)} % |")
    out += [
        ...
        "",
        "Intervalles par grappes de dates ; incertitude de Monte-Carlo des références ± 0,7 pt.",
    ]
    if be["ic"] is not None:
        au_marche = be["taux"] - ref["marche"]["bande"]
        au_bruit = ref["iid"]["bande"] - be["taux"]
        out += ["", (
            f"**`BANDE` vaut {fr(be['taux'], 1)} % ± {fr(be['ic'], 1)} pt**, soit "
            f"{signe(au_marche, 1)} pt par rapport à la marche aléatoire — écart qui "
            f"{'dépasse' if abs(au_marche) > be['ic'] else 'reste dans'} l'intervalle — et "
            f"{fr(au_bruit, 1)} pt sous le bruit i.i.d. — écart qui "
            f"{'dépasse' if abs(au_bruit) > be['ic'] else 'reste dans'} l'intervalle.")]
    out += ["", "### La matrice de passage", "",
            ("> Ligne : l'état de la clôture de la décision dans sa bande. Valeurs : la part des "
             "clôtures suivantes sous le bord bas · dans la bande · au-dessus du bord haut."), "",
            ("| Depuis | Bruit i.i.d. | Marche aléatoire | Observé 2021 | Observé 2022 "
             "| Observé, audit |"),
            "|---|---|---|---|---|---|"]
    for i, libelle in enumerate(("sous le bord bas", "dans la bande", "au-dessus du bord haut")):
        cellules = [" · ".join(fr(x, 1) for x in m["lignes"][i])
                    for m in (ref["iid"], ref["marche"], mat["etalonnage"], mat["narree"],
                              mat["audit"])]
        out.append(f"| **{libelle}** | " + " | ".join(cellules) + " |")
    out += ["", ("Les références et la matrice de 2021 recalculées **concordent** avec les "
                 "nombres publiés au README." if not ecarts else
                 f"**{len(ecarts)} nombres des références ne concordent pas** avec le README, "
                 "qui n'est pas corrigé après coup.")]
    return out


def section_exposition(b):
    t_u, t_t = b["temoins"]["UNIVERS"], b["temoins"]["TOP10"]
    appariee = b["appariee"][-1]
    exposition_pt, selection_pt = appariee - b["ref_fin"], b["base"] - appariee
    paire = b["paires"]["appariee"]
    diff_3 = b["paires"]["experience_3"]
    out = [
        "", "## 9. L'exposition séparée de la sélection — T2", "",
        "### La référence à exposition appariée", "",
        f"| | Base 100 au {b['fin']} |", "|---|---|",
        f"| `{REFERENCE}` | {fr(b['ref_fin'])} |",
        f"| Référence à exposition appariée | {fr(appariee)} |",
        f"| **Le portefeuille** | **{fr(b['base'])}** |",
        "",
        (f"Sur les {signe(b['base'] - b['ref_fin'])} points d'écart à `{REFERENCE}`, "
         f"**{signe(exposition_pt)} points reviennent à l'exposition** et "
         f"**{signe(selection_pt)} points à la sélection**. La tracking error contre la "
         f"référence appariée vaut {fr(paire['te'])} %/an, soit un effet minimal détectable de "
         f"± {fr(Z95 * paire['te'], 1)} points : la sélection "
         f"{'reste dans' if abs(selection_pt) <= Z95 * paire['te'] else 'dépasse'} son effet "
         "minimal détectable."),
        "",
        "### Les bêtas par semestre", "",
        "| | Premier semestre | Second semestre |", "|---|---|---|",
        f"| 2022 | {fr(b['betas'][0], 3)} | {fr(b['betas'][1], 3)} |",
        (f"| 2021, recalculé — publié {fr(PROJECTIONS_PUBLIEES['betas_semestres']['premier'], 3)}"
         f" et {fr(PROJECTIONS_PUBLIEES['betas_semestres']['second'], 3)} "
         f"| {fr(b['betas_p'][0], 3)} | {fr(b['betas_p'][1], 3)} |"),
        "",
        "### Le témoin aléatoire", "",
        (f"> {TIRAGES_TEMOIN} portefeuilles aux mêmes dates et aux mêmes montants que le "
         "portefeuille réel, valeurs tirées au hasard. **Le réservoir `UNIVERS` fait foi**, "
         "déclaré avant la première séance."), "",
    ...
    for t in (t_u, t_t):
        ...
        out.append(f"| `{t['reservoir']}` | {foi} "
                   f"| {fr(t['moyenne'])} | {fr(t['sd'])} pt | ± {fr(t['emd'], 1)} pt "
                   f"| {fr(t['reel'])} | {fr(t['rang'], 1)} % | {signe(t['z'])} |")
    lecture = ("**indiscernable** d'un choix de valeurs au hasard" if abs(t_u["z"]) <= Z95
               else "**au-delà** de ce que le hasard donne à 95 %")
    out += ["", (
        f"Le portefeuille dépasse **{fr(t_u['rang'], 1)} %** des témoins du réservoir qui fait "
        f"foi (z = {signe(t_u['z'])}) : il est {lecture} aux mêmes dates et montants.")]
    out += [
        "", "### La différence avec l'expérience 3, neutralisée", "",
        "| | |", "|---|---|",
        f"| Écart apparié, base 100 | {signe(b['base'] - b['base_3'])} pt |",
        f"| Bêta de la différence contre `{REFERENCE}` | {fr(diff_3['beta'], 3)} |",
        (f"| Alpha de la différence, bêta neutralisé | {signe(diff_3['alpha'])} %/an "
         f"± {fr(diff_3['ic_alpha'], 1)} |"),
        f"| Écart-type du résidu | {fr(diff_3['te_residu'])} %/an |",
        "",
        (f"Une fois le bêta de {fr(diff_3['beta'], 3)} retiré, l'alpha de la différence "
         f"{'contient' if contient_zero(diff_3['alpha'], diff_3['ic_alpha']) else 'exclut'} zéro."),
    ]
    return out


def section_cadence(b):
    cadence, regle, total = (b["paires"]["cadence"], b["paires"]["regle"],
                             b["paires"]["experience_3"])
    out = [
        "", "## 10. La règle et sa cadence — T3", "",
        ("> Le tableau de toutes les différences entre les expériences 3 et 4 est au "
         "[protocole](README.md#toutes-les-différences-entre-les-expériences-3-et-4-déclarées). "
         "`MENSUEL` applique la règle de l'expérience 4 au calendrier de l'expérience 3."), "",
        f"| | Base 100 au {b['fin']} | Part investie | Bêta | Ordres | Frais |",
        "|---|---|---|---|---|---|",
        (f"| **Expérience 5** — règle 4, quotidienne | **{fr(b['base'])}** "
         f"| {fr(b['part_investie'], 1)} % | {fr(b['regression']['beta'], 3)} "
         f"| {len(b['ordres'])} | {fr(b['frais'])} {EURO} |"),
        (f"| `MENSUEL` — règle 4, mensuelle | {fr(b['base_m'])} "
         f"| {fr(b['part_investie_m'], 1)} % | {fr(b['regression_m']['beta'], 3)} "
         f"| {len(b['ordres_m'])} | {fr(b['frais_m'])} {EURO} |"),
        (f"| [Expérience 3](../experience_3/bilan-2022.md) — règle 3, mensuelle "
         f"| {fr(b['base_3'])} | {fr(b['part_investie_3'], 1)} % "
         f"| {fr(b['regression_3']['beta'], 3)} | — | {fr(FRAIS_EXPERIENCE_3)} {EURO} |"),
        "",
        ("| Écart | Ce qu'il mesure | Base 100 | Écart-type de la différence | EMD "
         "| Bêta de la différence | Écart-type du résidu |"),
        "|---|---|---|---|---|---|---|",
    ]
    for libelle, mesure, valeur, p in (
            ("Expérience 5 − `MENSUEL`", "la **cadence**", b["base"] - b["base_m"], cadence),
            ("`MENSUEL` − expérience 3", "la **règle**, avec l'admission et les divisions",
             b["base_m"] - b["base_3"], regle),
            ("Expérience 5 − expérience 3", "les deux", b["base"] - b["base_3"], total)):
        out.append(f"| {libelle} | {mesure} | **{signe(valeur)} pt** | {fr(p['te'])} %/an "
                   f"| ± {fr(Z95 * p['te'], 1)} pt | {fr(p['beta'], 3)} "
                   f"| {fr(p['te_residu'])} %/an |")
    ecart_part = b["part_investie"] - b["part_investie_m"]
    out += ["", (
        f"**À règle égale, la cadence quotidienne déplace la part investie de "
        f"{signe(ecart_part, 1)} points** — {fr(b['part_investie'], 1)} % contre "
        f"{fr(b['part_investie_m'], 1)} %. L'écart de performance dû à la cadence, "
        f"{signe(b['base'] - b['base_m'])} points, "
        f"{'reste dans' if abs(b['base'] - b['base_m']) <= Z95 * cadence['te'] else 'dépasse'} "
        f"son effet minimal détectable de ± {fr(Z95 * cadence['te'], 1)} points.")]
    return out


def section_fantome_sensibilite_conventions(b):
    ecart_f = b["base"] - b["base_f"]
    mde_f = Z95 * b["paires"]["fantome"]["te"]
    out = [
        "", "## 11. Le fantôme `SANS-P20`", "",
        f"| | Base 100 au {b['fin']} | Ordres | Frais | Part investie |", "|---|---|---|---|---|",
        (f"| **Le portefeuille** | **{fr(b['base'])}** | {len(b['ordres'])} "
         f"| {fr(b['frais'])} {EURO} | {fr(b['part_investie'], 1)} % |"),
        (f"| Le fantôme `SANS-P20` | {fr(b['base_f'])} | {len(b['ordres_f'])} "
         f"| {fr(b['frais_f'])} {EURO} | {fr(b['part_investie_f'], 1)} % |"),
        "",
        (f"L'écart vaut {signe(ecart_f)} points, pour un effet minimal détectable de "
         f"± {fr(mde_f, 1)} : il {'reste dans' if abs(ecart_f) <= mde_f else 'dépasse'} "
         "son effet minimal détectable."),
        "", "## 12. La sensibilité aux paramètres — mesurée, pas arbitrée", "",
...
def section_dimensionnement(b):
    ...
    champs = (("Tracking error (%/an)", "te", 2), ("Bêta", "beta", 3),
              ("Part investie (%)", "part", 1), ("Séances en espèces", "vides", None),
              ("Ordres", "ordres", None), ("Frais (€)", "frais", 2),
              ("Durée médiane (séances)", "duree", 1),
              ("Lignes détenues en fin d'année", "ouvertes", None),
              ("Signaux perdus faute de créneau", "perdus", None))
    out = ["", "## 14. Le dimensionnement, confronté — T4", ""]
    for nom, cle_p, cle_r, titre in (("quotidien", "p_q", "r", "Expérience 5"),
                                    ("mensuel", "p_m", "r_m", "`MENSUEL`"),
                                    ("sans_p20", "p_f", "r_f", "`SANS-P20`")):
        out += [f"### {titre}", "",
        ...
        for libelle, cle, decimales in champs:
            ...
            recalc = b[cle_p]["n_ordres"] if cle == "ordres" else b[cle_p][cle]
            realise = b[cle_r]["n_ordres"] if cle == "ordres" else b[cle_r][cle]

            def texte(v, d=decimales):
                return str(v) if d is None or v is None else fr(v, d)

            out.append(f"| {libelle} | {texte(publie)} | {texte(recalc)} "
                       f"| {'✗' if (f'projection {nom}', cle) in ecarts else '✓'} "
                       f"| {texte(realise)} |")
        out.append("")
    p, q = b["paires_p"], b["paires"]
    out += ["### Les écarts appariés", "",
            "| Écart-type de la différence | Publié, 2021 | Recalculé, 2021 | Réalisé, 2022 |",
            "|---|---|---|---|",
            (f"| contre `SANS-P20` | {fr(PROJECTIONS_PUBLIEES['paires']['fantome'])} "
             f"| {fr(p['fantome']['te'])} | {fr(q['fantome']['te'])} |"),
            (f"| contre `MENSUEL` | {fr(PROJECTIONS_PUBLIEES['paires']['mensuel'])} "
             f"| {fr(p['mensuel']['te'])} | {fr(q['cadence']['te'])} |"),
            (f"| contre `MENSUEL`, bêta neutralisé "
             f"| {fr(PROJECTIONS_PUBLIEES['paires']['mensuel_neutralise'])} "
             f"| {fr(p['mensuel']['te_residu'])} | {fr(q['cadence']['te_residu'])} |"),
            (f"| contre la référence appariée | {fr(PROJECTIONS_PUBLIEES['paires']['appariee'])} "
             f"| {fr(p['appariee']['te'])} | {fr(q['appariee']['te'])} |"),
            ""]
    erreur_5 = abs(math.log(b["te"] / TE_DECLAREE))
    erreur_4 = abs(math.log(b["te"] / 15.58))
    reg = b["regression"]
    out += [
        "### La tracking error déclarée", "",
        "| | Déclarée | Réalisée | Écart relatif, en logarithme |", "|---|---|---|---|",
        (f"| **Expérience 5** — projetée sur la règle elle-même | {fr(TE_DECLAREE)} %/an "
         f"| {fr(b['te'])} %/an | {fr(erreur_5, 3)} |"),
        (f"| Expérience 4 — empruntée à l'expérience 3 | 15,58 %/an | {fr(b['te'])} %/an "
         f"| {fr(erreur_4, 3)} |"),
        "",
        (("La projection sur la règle elle-même n'est **ni plus ni moins proche** du réalisé "
          f"que l'emprunt, à {fr(abs(erreur_5 - erreur_4), 3)} près en logarithme"
          if abs(erreur_5 - erreur_4) < 0.01 else
          f"La projection sur la règle elle-même est "
          f"{'plus proche' if erreur_5 < erreur_4 else 'plus éloignée'} du réalisé que "
          "l'emprunt")
         + " ; la confrontation n'est pas aveugle, le réalisé de 2022 étant connu depuis "
         "l'expérience 4. Le gain porte sur les grandeurs qui n'étaient pas projetées du tout."),
        "", "### L'alpha de régression", "", "| | |", "|---|---|",
        f"| Bêta contre `{REFERENCE}` | **{fr(reg['beta'], 3)}** |",
        (f"| Alpha de régression, annualisé | **{signe(reg['alpha'])} %/an** "
         f"± {fr(reg['ic_alpha'], 1)} |"),
        f"| Effet minimal détectable déclaré | ± {fr(Z95 * TE_DECLAREE, 1)} pt |",
        f"| Effet minimal détectable réalisé | ± {fr(b['mde'], 1)} pt |",
    ]
    return out


def section_conclusion(b):
    verdicts, verdicts_4 = b["verdicts"], b["verdicts_4"]
    separent = [lib for cle, lib, *_r in COMPARAISONS if verdicts[cle] == "sépare"]
    tombes = [lib for cle, lib, *_r in COMPARAISONS
              if survie(verdicts_4[cle], verdicts[cle]) == "**tombe**"]
    t_u = b["temoins"]["UNIVERS"]
    be = b["bande_echantillon"]
    appariee = b["appariee"][-1]
    alpha = b["base"] - b["ref_fin"]
    ...
    return [
        ...
        ("- que le portefeuille remesuré est **celui de l'expérience 4**, ordre par ordre — § 1 ;"),
        ("- **quels verdicts de l'expérience 4 tombent** sous une incertitude correcte : "
         + (", ".join(tombes) if tombes else "aucun") + " ; et ce qui sépare son issue sous la "
         "convention déclarée : " + (", ".join(separent) if separent else "aucun élément")
         + " — § 7 ;"),
        (f"- que `BANDE`, {fr(be['taux'], 1)} %, se situe entre ses deux références, "
         f"{fr(b['references']['marche']['bande'], 1)} % et "
         f"{fr(b['references']['iid']['bande'], 1)} % — § 8 ;"),
        (f"- que {signe(appariee - b['ref_fin'])} des {signe(alpha)} points d'écart à "
         f"`{REFERENCE}` reviennent à l'exposition — § 9 ;"),
        (f"- que la cadence quotidienne, à règle égale, déplace la part investie de "
         f"{signe(b['part_investie'] - b['part_investie_m'], 1)} points — § 10 ;"),
        (f"- que {len(ETALONNAGE_PUBLIE) + 0} taux d'étalonnage, les références et les projections "
         "publiées avant la première séance "
         + ("sont tous retrouvés par le moteur" if not nb_ecarts
            else f"sont retrouvés à {nb_ecarts} écarts près, publiés") + " — § 6, 8, 14."),
        "", "**Elle n'établit pas** :", "",
        (f"- que la règle sélectionne mieux que le hasard : rang {fr(t_u['rang'], 1)} %, "
         f"z = {signe(t_u['z'])}"
         + (", dans ce que le hasard donne à 95 % ;" if abs(t_u["z"]) <= Z95 else
            " — au-delà du hasard à 95 %, mais sur une règle formulée après avoir vu 2022, "
            "donc sans valeur probante ;")),
        ("- quoi que ce soit d'aveugle sur 2022 : l'année, le portefeuille et plusieurs aperçus "
         "des résultats étaient connus avant d'écrire les conventions ;"),
        ("- que ces conventions sépareront quelque chose sur une autre année. C'est ce qu'une "
         "expérience suivante, sur une année neuve, peut démontrer."),
        "", "---", "",
        (f"[← Protocole](README.md) · [Décembre]({RAPPORTS}/{ANNEE}-12.md) · "
         f"[Janvier]({RAPPORTS}/{ANNEE}-01.md) · [L'expérience 4](../experience_4/bilan-2022.md) · "
         "[Sa revue](../experience_4/review.md)"),
    ]
...
def bilan_annuel(ctx, b):
    out = [
        ...
        (f"> [Expérience 5](README.md) · l'expérience 4 remesurée · **{signe(b['base'] - 100)} %** "
         f"contre **{signe(b['ref_fin'] - 100)} %** pour {REFERENCE}"), "",
        ("> ⚠️ **Le portefeuille est celui de l'expérience 4**, et le "
         "[protocole](README.md#-ce-que-remesurer-2022-peut-établir-et-ce-quil-ne-peut-pas) "
         "déclarait avant la première séance que sa performance n'était pas l'objet. Ce que ce "
         "bilan établit est aux sections 7 à 10 et 14."), "", "---", "",
        *section_compte(b), *section_positions(b), *section_univers(ctx, b),
        *section_etalonnage(b), *section_issues(b), *section_bande(b),
        *section_exposition(b), *section_cadence(b),
        *section_fantome_sensibilite_conventions(b), *section_dimensionnement(b),
...
def construire_bilan(ctx):
    """Rejoue, controle, remesure : tout ce que le bilan et la console publient."""
    seances = [ctx["execution"][d] for d in ctx["jours_narres"]]
    debut, fin = seances[0], seances[-1]
    simulation = simuler(ctx, ctx["jours_narres"], seances)
    ordres, signaux, valeurs, journal, _registre = simulation
    controle = controler_reproduction(ctx["args"].repertoire, ordres, valeurs)

    r = resumer(ctx, simulation, seances)
    r_f = resumer(ctx, simuler(ctx, ctx["jours_narres"], seances, avec_p20=False), seances)
    r_m = resumer(ctx, simuler(ctx, decisions_mensuelles(ctx, ctx["jours_narres"]), seances),
                  seances)
    projection = ctx["jours_projection"]
    seances_p = [ctx["execution"][d] for d in projection]
    p_q = resumer(ctx, simuler(ctx, projection, seances_p), seances_p)
    p_m = resumer(ctx, simuler(ctx, decisions_mensuelles(ctx, projection), seances_p), seances_p)
    p_f = resumer(ctx, simuler(ctx, projection, seances_p, avec_p20=False), seances_p)

    experience_3 = charger_experience_3(ctx["args"].repertoire, seances)
    base, indice = r["base"], r["indice"]
    base_3 = [100 * experience_3[j][1] / DOTATION for j in seances]
    ref100 = dict(zip(seances, indice, strict=True))
    ...
    resultat_phases = phases(lignes)
    rejets = holm({cle: resultat_phases[cle]["phases"][0]["p"] for cle, *_r in COMPARAISONS})

    evs, rgs = ctx["evaluations"], ctx["rangs"]
    appariee = reference_appariee(ctx, valeurs, seances)
    appariee_p = reference_appariee(ctx, p_q["valeurs"], seances_p)
    par_sort = {}
    ...
    return {
        ...
        "ref100": ref100, "experience_3": experience_3, "controle": controle,
        "ordres": ordres, "ordres_f": r_f["ordres"], "ordres_m": r_m["ordres"],
        "signaux": signaux, "journal": journal,
        "r": r, "r_f": r_f, "r_m": r_m, "p_q": p_q, "p_m": p_m, "p_f": p_f,
        "total": valeurs[fin][2], "base": base[-1], "base_f": r_f["base"][-1],
        "base_m": r_m["base"][-1], "base_3": base_3[-1], "ref_fin": indice[-1],
        "nue": 100 * (nue[fin]["close"] / nue[debut]["close"] - 1),
        "achats": sum(1 for o in ordres if o["SENS"] == "ACHAT"),
        "frais": r["frais"], "frais_f": r_f["frais"], "frais_m": r_m["frais"],
        "repli": 100 * repli, "creux": creux, "especes_fin": valeurs[fin][0],
        "part_investie": r["part"], "part_investie_f": r_f["part"],
        "part_investie_m": r_m["part"], "seances_vides": r["vides"],
        "part_investie_3": statistics.fmean(
            100 * experience_3[j][0] / experience_3[j][1] for j in seances),
        "positions": r["positions"], "par_sort": par_sort,
        "jours_pleins": len({s["DATE_DECISION"] for s in par_sort.get("FAUTE DE CRENEAU", [])}),
        "lignes_issues": lignes, "phases": resultat_phases, "holm": rejets,
        ...
        "verdicts_4": {cle: verdict_experience_4(resultat_phases[cle]["phases"][0])
                       for cle, *_r in COMPARAISONS},
        "quotidien": {cle: grappes(lignes, pop, grp) for cle, *_r, pop, grp in COMPARAISONS},
        "bande_echantillon": proportion_grappes(echantillon, lambda li: li["BANDE_SUIVANTE"]),
        "bande_quotidien": proportion_grappes(lignes, lambda li: li["BANDE_SUIVANTE"]),
        "etats": {
        ...
        "matrices": {
            "etalonnage": lire_matrice(matrice_bande(ctx, ctx["jours_etalonnage"],
                                                     FIN_ETALONNAGE)),
            "narree": lire_matrice(matrice_bande(ctx, ctx["jours_narres"], FIN_SERIE)),
        ...
        "te": r["te"], "mde": Z95 * r["te"], "regression": r["regression"],
        "regression_m": r_m["regression"], "regression_3": regression(base_3, indice),
        "paires": {
            "fantome": ecart_apparie(base, r_f["base"]),
            "cadence": ecart_apparie(base, r_m["base"], indice),
            "regle": ecart_apparie(r_m["base"], base_3, indice),
            "experience_3": ecart_apparie(base, base_3, indice),
            "appariee": ecart_apparie(base, appariee, indice),
        },
        "paires_p": {
            "fantome": ecart_apparie(p_q["base"], p_f["base"]),
            "mensuel": ecart_apparie(p_q["base"], p_m["base"], p_q["indice"]),
            "appariee": ecart_apparie(p_q["base"], appariee_p),
        },
        "appariee": appariee,
        "betas": betas_semestres(base, indice, seances, FIN_SEMESTRE),
        "betas_p": betas_semestres(p_q["base"], p_q["indice"], seances_p, FIN_SEMESTRE_2021),
        "temoins": {res: temoin(ctx, r["positions"], fin, valeurs[fin][2], res)
                    for res in ("UNIVERS", "TOP10")},
        "temoins_p": {res: temoin(ctx, p_q["positions"], seances_p[-1],
                                  p_q["valeurs"][seances_p[-1]][2], res)
                      for res in ("UNIVERS", "TOP10")},
        "episodes_candidats": episodes(ctx, lambda t, j: est_candidat(evs[j][t], rgs[j].get(t))),
...
def bloc_mensuel(b, mois):
    ...
    lignes += ["", (f"Portefeuille au {fin_mois} : {fr(total)} EUR ({nombre} lignes, "
                    ...
                    f"{fr(100 * total / b['dotation'])} {MEDIAN} MENSUEL "
                    f"{fr(100 * b['valeurs_m'][fin_mois][2] / b['dotation'])} {MEDIAN} "
                    f"{REFERENCE} {fr(b['ref100'][fin_mois])}")]
...
=== Bilan au {b['fin']} ===
  ...
  Controle de reproduction {nombre} ordres, {fr(valeur)} EUR, identiques a l'experience 4
...
IC95 +/- {fr(reg['ic_alpha'], 1)} pt)
  Exposition / selection  {signe(appariee - b['ref_fin'])} pt / {signe(b['base'] - appariee)} pt \
{MEDIAN} betas semestriels {fr(b['betas'][0], 3)} puis {fr(b['betas'][1], 3)}
  Cadence (MENSUEL)       {signe(b['base'] - b['base_m'])} pt {MEDIAN} part investie \
{fr(b['part_investie'], 1)} % contre {fr(b['part_investie_m'], 1)} %""")
    print("  Issues, grappes, phase 0 :")
    for cle, *_r in COMPARAISONS:
        c = b["phases"][cle]["phases"][0]
        print(f"    {cle:<10s} {signe(c['difference'])} +/- {fr(c['ic'])} pt {MEDIAN} "
              f"p {fr(c['p'], 3)} {MEDIAN} Holm {'oui' if b['holm'][cle] else 'non'} "
              f"{MEDIAN} phases "
              f"{b['phases'][cle]['n_exclut']}/{PHASES} {MEDIAN} {b['verdicts_4'][cle]} -> "
              f"{b['verdicts'][cle]}")
    be = b["bande_echantillon"]
    print(f"  BANDE                   {fr(be['taux'], 1)} % +/- {fr(be['ic'], 1)} "
          f"(i.i.d. {fr(b['references']['iid']['bande'], 1)} %, marche "
          f"{fr(b['references']['marche']['bande'], 1)} %)")
...
def analyser_arguments():
    ...
    parser = argparse.ArgumentParser(
        description=f"Journal de l'experience 5 : l'experience 4 sur {ANNEE}, remesuree.")
...
def main():
    ...
    ecrits = ["evaluations.csv", "top10.csv", "ordres.csv", "signaux.csv", "issues.csv",
              "portefeuille.csv", "fantome.csv", "mensuel.csv", "phases.csv", "temoin.csv"]
